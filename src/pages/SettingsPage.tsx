...
const priorityRows: Array<{ helper: string; key: TaskPriority; title: string }> = [
  { helper: "Used for low effort or non-urgent work.", key: "low", title: "Low priority" },
  { helper: "Used for normal day-to-day tasks.", key: "medium", title: "Medium priority" },
  { helper: "Used for urgent or important tasks.", key: "high", title: "High priority" },
];

export function SettingsPage() {
  const { prioritySettings, resetPrioritySettings, updatePrioritySetting } = usePrioritySettings();
  ...
  function updateVariant(priority: TaskPriority, variant: PrioritySetting["variant"]) {
    updatePrioritySetting(priority, {
      ...prioritySettings[priority],
      variant,
    });
          ...
          <div className="space-y-4 rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  Customize the label and badge color used across task cards, forms, filters, and
                  backlog.
                </p>
              </div>
                  ...
                  >
                    ...
                    <label className="space-y-1.5">
                      ...
                      <input
                        ...
                        maxLength={24}
                        onChange={(event) => updateLabel(priorityRow.key, event.target.value)}
                        value={setting.label}
                      />
                    </label>
